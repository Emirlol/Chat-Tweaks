...
//		val messageIndexUnderMouse = getMessageIndex(toChatLineX(mouseX.toDouble()), toChatLineY(mouseY.toDouble())) //n
	...
	fun scroll(lines: Int) {
		...
		scrolledLines = scrolledLines.coerceAtMost(visibleMessages.size - visibleLineCount)
	}
	...
	/**
	 * @return The index of the message. This is the top-most message out of multiple if a line was split into multiple lines.
	 */
	fun getMessageIndex(chatLineX: Double, chatLineY: Double): Int {
		var i = getMessageLineIndex(chatLineX, chatLineY)
		while (i >= 0) {
			if (visibleMessages[i].endOfEntry) return i
			i--
		}
		return i
	}
	...
	/**
	 * @return -1 if message line index is not obtainable, line index in the visibleMessages list otherwise.
	 */
	fun getMessageLineIndex(chatLineX: Double, chatLineY: Double): Int {
		if (!isChatFocused || isChatHidden
			|| chatLineX < 0 || chatLineX > scaledWidth
			|| chatLineY < 0 || chatLineY >= min(getVisibleLineCount(), visibleMessages.size)
		) return -1
		val j = MathHelper.floor(chatLineY + scrolledLines)
		return if (j in 0 until visibleMessages.size) j else -1
	}
	...
	fun addVisibleMessage(message: ChatHudLine, isChatFocused: Boolean) {
		...
		for (orderedText in list) {
			...
			val endOfEntry = orderedText === list.last()
			visibleMessages.addFirst(VisibleChatHudLine(message, orderedText, endOfEntry))
		}
	}

	//Converts the x and y values to chat line x and y values based on this chat box's scale
	fun toChatLineX(x: Double) = x / data.scale

	fun toChatLineY(y: Double) = (client.window.scaledHeight - y) / (data.scale * lineHeight)
